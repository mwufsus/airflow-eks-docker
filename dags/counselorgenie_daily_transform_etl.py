...
E.g. final_applications.sql -> /prod/procurement/Application/
"""

with open(os.path.join(config_dir, "counselorgenie_etl_final.yml")) as etl_file:
    config_file = yaml.safe_load(etl_file)

    for model in config_file["transforms"]:

        etl_transform_task = PostgresOperator(
            dag=dag,
        ...
        etl_transform_task >> vacuum_task >> analyze_task
